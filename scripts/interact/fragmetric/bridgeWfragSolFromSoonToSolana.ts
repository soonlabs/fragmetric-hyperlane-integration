...
export async function bridgeWfragSolFromSoonToSolana({
  conn,
  sender,
  recipient,
  bridgeAmount,
...
async function main() {
  ...
  const { tx, signers } = await bridgeWfragSolFromSoonToSolana({
    conn,
  ...
  await sendTxn(conn.sendTransaction(tx, [sender, ...signers]), "bridge wfragSOL");
}
